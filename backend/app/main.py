...
def admin_operations_reports(
    ...
    total_cashback_given = round(sum(row.cashback_amount or 0.0 for row in rows), 2)
    total_cashback_all_users = round(
        float(db.query(func.coalesce(func.sum(Transaction.cashback_amount), 0.0)).scalar() or 0.0),
        2,
    )
    ...
    summary = AnalystReportSummary(
        ...
        total_cashback_all_users=total_cashback_all_users,
